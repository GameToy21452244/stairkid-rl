...
class ShaftEnvConfig:
    ...
    startup_acceleration_multiplier: float = 2.4
    acceleration_curve_exponent: float = 2.0
    startup_impulse_speed: float = 60.0
    reversal_brake_speed: float = 30.0
    enable_arcade_horizontal_control: bool = False
    ...
    conveyor_spawn_probability: float = 0.0
    ...
    flipping_spawn_probability: float = 0.0
    minimum_normal_platforms_between_specials: int = 1
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.conveyor_spawn_probability <= 1.0:
            raise ValueError("conveyor_spawn_probability 必須介於 0 與 1。")
        if self.conveyor_spawn_probability > 0 and not self.enable_conveyor:
            raise ValueError("conveyor_spawn_probability > 0 需要 enable_conveyor。")
        if not 0.0 <= self.flipping_spawn_probability <= 1.0:
            raise ValueError("flipping_spawn_probability 必須介於 0 與 1。")
        if self.flipping_spawn_probability > 0 and not self.enable_flipping:
            raise ValueError("flipping_spawn_probability > 0 需要 enable_flipping。")
        if self.minimum_normal_platforms_between_specials < 0:
            raise ValueError("minimum_normal_platforms_between_specials 不可小於 0。")
...
FIDELITY_V1_VERSION = "ns-shaft-sim-fidelity-v1"


def build_fidelity_v1_config(**overrides) -> ShaftEnvConfig:
    fps = overrides.get("fps", 10)
    base = {
        "environment_version": FIDELITY_V1_VERSION,
        "fps": fps,
        "physics_hz": 60,
        "allow_manual_60hz_control": (fps == 60),
        ...
        "enable_arcade_horizontal_control": True,
        "enable_swept_edge_collision": True,
        # Health & Spikes
        "enable_health": True,
        "initial_health_segments": 12,
        "max_health_segments": 12,
        "normal_platform_heal_segments": 1,
        "enable_spikes": True,
        "spike_damage_segments": 5,
        "spike_spawn_probability": 0.08,
        "initial_safe_normal_platforms": 4,
        "minimum_normal_platforms_between_spikes": 5,
        # Spring
        "enable_spring": True,
        "spring_jump_velocity": 240.0,
        "spring_spawn_probability": 0.06,
        "minimum_normal_platforms_before_spring": 3,
        # Conveyor
        "enable_conveyor": True,
        "conveyor_velocity_delta": 100.0,
        "conveyor_spawn_probability": 0.10,
        # Flipping
        "enable_flipping": True,
        "flipping_active_seconds": 0.25,
        "flipping_inactive_seconds": 1.0,
        "flipping_spawn_probability": 0.06,
        # Special spacing
        "minimum_normal_platforms_between_specials": 1,
    }
    base.update(overrides)
    return ShaftEnvConfig(**base)
