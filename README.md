...
觀測 v2 把既有辨識結果轉為 64 個 `float32` 特徵，範圍固定在 `[-1, 1]`：
...
可在設定檔調整，但改動後會同時改變 observation space。動作空間為：
...
互切。角色上升時會排除正下方的起跳平台並提早選擇下一落點；若起跳平台是
彈簧且暫時看不到下一平台，則沿現有水平速度離開彈簧，避免原地反覆彈跳。
尖刺進入設定的
...
尖刺避讓、控制更新慢與彈簧重複落點；目前已加入目標空間重取、單步換向煞車、
背景視窗安全快取、上升期改選下一平台與彈簧脫離。必須再實機驗證修正版，才
